...
<form action="<?php echo e($action); ?>" method="POST" class="form-horizontal">
	...
	<div class="box-body">
		...
		<div class="form-group required">
			...
			<div class="col-sm-9">
				<?php if($method == 'PUT'): ?>
					<input type="hidden" name="item_id" value="<?php echo e($stockTransportDetail->item_id); ?>" />
					<textarea class="form-control" readonly><?php echo e($stockTransportDetail->item->sku); ?>  - <?php echo e($stockTransportDetail->item->name); ?> - <?php echo e($stockTransportDetail->ref_code); ?> (Item Maksimal :<?php echo e($stockTransportDetail->item_outstanding); ?>)</textarea>

				<?php elseif($method == 'POST'): ?>
					<select class="form-control" name="item_id" required>
	                    <option value="">-- Pilih Item SKU --</option>
						<?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
	                        <option data-ref-code="<?php echo e($item->ref_code); ?>" value="<?php echo e($item->item_id); ?>" <?php if(old('item_id', $stockTransportDetail->item_id) == $item->item_id): ?><?php echo e('selected'); ?><?php endif; ?>>
	                        	<?php echo e($item->item->sku); ?> - <?php echo e($item->item->name); ?> - <?php echo e($item->ref_code); ?> (Item Maksimal :<?php echo e($item->item_outstanding); ?>) 
							</option>
		...
		<div class="form-group required">
			...
			<div class="col-sm-9">
				<input type="text" name="ref_code"  class="form-control" value="<?php echo e(old ('ref_code', $stockTransportDetail->ref_code)); ?>" readonly />                	
                <?php if($stockTransportDetail->header->type == 'outbound'): ?>
		...
		<div class="form-group required">
			...
			<div class="col-sm-9">
				<input type="number" step="0.01" name="plan_qty" id="qty_change_std" class="form-control" placeholder="Qty" value="<?php echo e(old('plan_qty', $stockTransportDetail->plan_qty)); ?>" required>
			</div>
		</div>
		<div class="form-group">
			<label for="uom_id" class="col-sm-3 control-label">UOM</label>
			<div class="col-sm-9">
				<input type="text" name="uom_name" value="<?php echo e(old('uom_name', @$stockTransportDetail->uom->name)); ?>" readonly class="form-control">
				<input type="hidden" name="uom_id" value="<?php echo e(old('uom_id', @$stockTransportDetail->uom_id)); ?>">
			</div>
		</div>
		<div class="form-group required">
			<label for="weight" class="col-sm-3 control-label">Weight</label>
			<div class="col-sm-9">
				<input type="number" step="0.01" name="plan_weight" class="form-control" placeholder="Weight" value="<?php echo e(old('plan_weight', $stockTransportDetail->plan_weight)); ?>" required readonly>
			</div>
		</div>
		<div class="form-group required">
			<label for="volume" class="col-sm-3 control-label">Volume</label>
			<div class="col-sm-9">
				<input type="number" step="0.01" name="plan_volume" class="form-control" placeholder="Volume" value="<?php echo e(old('plan_volume', $stockTransportDetail->plan_volume)); ?>" required readonly>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<input id="control_method" type="text" name="control_method" class="form-control" placeholder="Control Method" value="<?php echo e(old('control_method', $method == 'PUT' ? $stockTransportDetail->item->control_method->name : '')); ?>" readonly>
			</div>
		</div>
		<div class="form-group required <?php if($stockTransportDetail->header->type == 'outbound'): ?> hidden <?php endif; ?>">
			<label for="control_date" class="col-sm-3 control-label">Control Date</label>
			<div class="col-sm-9">
				<input id="control_date" type="text" name="control_date" class="form-control datepicker-normal" placeholder="Control Date" value="<?php echo e(old('control_date', empty($stockTransportDetail->control_date) ? $stockTransportDetail->header->created_at : $stockTransportDetail->control_date )); ?>" required>
			</div>
		</div>
	</div>
	<!-- /.box-body -->
