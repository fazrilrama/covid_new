	...
	<div class="box-body">
		<div class="row">
			<div class="col-sm-6">
				...
				<input type="hidden" id="method_form_stock_transport" name="method" value="{{ !empty($method) ? $method : '' }}">
				<div class="form-group required">
					<label for="advance_notice_id" class="col-sm-3 control-label">{{  __('lang.document_reference') }} #</label>
					<div class="col-sm-9">
						<select class="form-control" name="advance_notice_id" id="advance_notice_id" required>
							...
							@foreach($advance_notices as $advance_noticeValue)
								@if($advance_noticeValue->show == true)
									<option value="{{$advance_noticeValue->id}}" @if(old('advance_notice_id', $stockTransport->advance_notice_id) == $advance_noticeValue->id){{'selected'}}@endif>{{$advance_noticeValue->code}}</option>
								@endif
							@endforeach
						</select>
					</div>
				...
				<div class="form-group">
					<label for="warehouse_id" class="col-sm-3 control-label">{{  __('lang.warehouse') }}</label>
					<div class="col-sm-9">
						<input id="warehouse_name" type="text" class="form-control" placeholder="warehouse" @if($stockTransport->warehouse)
							value="{{old('warehouse_name', $stockTransport->warehouse->name)}}"
						@endif readonly required>

						<input id="warehouse_id" type="hidden" name="warehouse_id" class="form-control" placeholder="warehouse" value="{{old('warehouse_id', $stockTransport->warehouse_id)}}">
					</div>
				</div>
				...
				<div class="form-group required">
					...
					<div class="col-sm-9">
						<input id="transport_type_name" type="text" class="form-control" placeholder="{{ __('lang.type_transpotation') }}" @if($stockTransport->transport_type)
							value="{{old('transport_type_name', $stockTransport->transport_type->name)}}"
						@endif readonly required>

						<input id="transport_type_id" type="hidden" name="transport_type_id" class="form-control" placeholder="{{ __('lang.type_transpotation') }}" value="{{old('transport_type_id', $stockTransport->transport_type_id)}}">
					</div>
				</div>
				<div class="form-group required">
					<label for="ref_code" class="col-sm-3 control-label">{{ __('lang.customer_reference') }}</label>
					<div class="col-sm-9">
						<input id="ref_code" type="text" name="ref_code" class="form-control" placeholder="{{ __('lang.customer_reference') }}" value="{{old('ref_code', $stockTransport->ref_code)}}" readonly>
				...
				<div class="form-group required">
					...
					<div class="col-sm-9">
						<select class="form-control" name="shipper_id" id="shipper_id" readonly >
							...
							<option value="{{$party->id}}" @if($stockTransport->shipper_id == $party->id){{'selected'}}@endif>{{$party->name}}</option>
							@endforeach
						</select>
					</div>
				</div>
				<div class="form-group required">
					...
					<div class="col-sm-9">
						<input type="text" name="shipper_address" id="shipper_address" class="form-control" placeholder="Address" value="{{old('shipper_address', $stockTransport->shipper_address)}}" readonly >
					</div>
				</div>
				<div class="form-group required">
					...
					<div class="col-sm-9">
						<select class="form-control" name="origin_id" id="origin_id" readonly >
							...
							<option value="{{$id}}" @if($stockTransport->origin_id == $id){{'selected'}}@endif>{{$value}}</option>
							@endforeach
						</select>
					</div>
				</div>
				<div class="form-group required">
					...
					<div class="col-sm-9">
						<input type="text" name="etd" id="etd" class="form-control datepicker" placeholder="Est. Time Delivery" value="{{old('etd', $stockTransport->etd)}}" readonly >
					</div>
				</div>
				@if($method == 'PUT')
					<div class="form-group">
						...
						<div class="col-sm-9">
							<p class="form-control-static">{{ ($stockTransport->status == 'Pending') ? 'Planning' : ($stockTransport->status == 'Completed' ? 'Completed' : $stockTransport->status) }}</p>
						</div>
					</div>
				@endif
			</div>
			<div class="col-sm-6">
				<div class="form-group required">
					...
					<div class="col-sm-9">
						<select class="form-control" name="consignee_id" id="consignee_id" readonly >
							...
							<option value="{{$party->id}}" @if($stockTransport->consignee_id == $party->id){{'selected'}}@endif>{{$party->name}}</option>
							@endforeach
						</select>
					</div>
				</div>
				<div class="form-group required">
					...
					<div class="col-sm-9">
						<input type="text" name="consignee_address" id="consignee_address" class="form-control" placeholder="Address" value="{{old('consignee_address', $stockTransport->consignee_address)}}" readonly >
					</div>
				</div>
				<div class="form-group required">
					...
					<div class="col-sm-9">
						<select class="form-control" name="destination_id" id="destination_id" readonly >
							@foreach($cities as $id => $value)
								<option value="{{$id}}" @if($stockTransport->destination_id == $id){{'selected'}}@endif>{{$value}}</option>
							@endforeach
						</select>
					</div>
				</div>
				...
				<div class="form-group required">
					<label for="owner_id" class="col-sm-3 control-label">{{ __('lang.ownership') }}</label>
					<div class="col-sm-9">
						<select class="form-control" name="owner" id="owner" readonly >
							@foreach($parties as $key => $value)
								<option value="{{$value->id}}" @if($stockTransport->owner == $value->id){{'selected'}}@endif>{{$value->name}}</option>
							@endforeach
						</select>
					</div>
				</div>
				...
				<div class="form-group required">
					...
					<div class="col-sm-9">
					<input type="text" name="pic" id="pic" class="form-control" placeholder="PIC" value="{{old('pic', $stockTransport->pic)}}" readonly >
					</div>
				</div>
				...
				<div class="form-group required">
					...
					<div class="col-sm-9">
						<input type="text" name="eta" id="eta" class="form-control end-datepicker" placeholder="{{ __('lang.eta') }}" value="{{old('eta', $stockTransport->eta)}}" readonly >
					</div>
				</div>
				
				<div class="form-group required">
					<label for="pickup_order" class="col-sm-3 control-label">{{ __('lang.pickup_time') }}</label>
					<div class="col-sm-9">
						<input type="text" name="pickup_order" id="datetimepickergr" class="form-control " placeholder="" value="{{old('pickup_order', $stockTransport->pickup_order)}}"  >
					</div>
				</div>
				...
				<div class="form-group required">
                    ...
                    <div class="col-sm-9">
                        @if($method == 'PUT')
                        	<input type="text" id="employee_name" name="employee_name" readonly class="form-control" placeholder="Warehouse Officer" value="{{old('employee_name', $stockTransport->employee_name )}}">
                        @else
                        	<select class="form-control" name="employee_name" id="employee_name" required>
	                            <option value="" selected disabled>-- {{ __('lang.choose') }} {{ __('lang.warehouse_checker') }} --</option>
				...
				<!-- <div class="form-group">
					...
					<div class="col-sm-9">
						<input type="text" name="annotation" id="annotation" class="form-control" placeholder="Annotation" value="{{old('annotation', $stockTransport->annotation)}}" readonly >
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<input type="text" name="contractor" id="contractor" class="form-control" placeholder="Contractor" value="{{old('contractor', $stockTransport->contractor)}}" readonly >
					</div>
				</div> -->
				...
				<!-- <div class="form-group required">
					...
					<div class="col-sm-9">
						<input type="text" name="head_ds" id="head_ds" class="form-control" placeholder="Head Cabang/Subcabang" value="{{old('head_ds', $stockTransport->head_ds)}}" readonly >
					</div>
				</div> -->
