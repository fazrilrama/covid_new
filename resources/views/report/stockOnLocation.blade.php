...
<div class="row">
    <div class="col-md-12">
        <div class="box box-default">
            ...
            <form action="{{ route('report.stock_on_location') }}" method="POST" class="form-horizontal">
                {{ csrf_field() }}
                <div class="box-body">
                    <div class="row">
                        <div class="col-sm-6">
                                        ...
                                        @foreach($warehouses as $warehouse)
                                            <optgroup label="{{ $warehouse->warehouse->name }}">
                                                @foreach($warehouse->storages as $storage)
                                                    <option value="{{ $storage->id }}" {{ $request->storage_id == $storage->id ? 'selected' : '' }}>{{ $storage->code }}</option>
                                                @endforeach
                                            </optgroup>                                       
                                        @endforeach
                                    </select>
...
<input id="filter_data" type="hidden" value="{{ route('report.masterfilter') }}">
<input type="hidden" id="selected_sku" value="{{ $request->sku }}">
<input type="hidden" id="selected_ref" value="{{ $request->ref }}">
<input type="hidden" id="selected_control_date" value="{{ $request->control_date }}">
@endsection
@section('custom_script')
    ...
    <script>
        ...
        $("#obutn").click(function() {
            $(this).closest("form").attr("action", "{{ route('report.stock-on-location') }}").attr('method','get');     
        });
        $("#oEbutn").click(function() {
            $(this).closest("form").attr("action", "{{ route('report.stock_on_location') }}").attr('method', 'post');       
        });
    </script>
