        ...
        <div class="box">
            ...
            <div class="box-body table-responsive no-padding">
                <table class="table table-bordered table-hover no-margin activity">
                    ...
                    <tr>
                        <tr>
                            <td rowspan="3" style="text-align: center; vertical-align: middle">{{$item->warehouse_name}}</td>
                            <td>{{ $item->code }}</td>
                            <td>{{$item->created_at}}</td>
                            <td>{{$item->updated_at}}</td>
                            <td>{{floor ($item->times_spent/1440)}}</td>
                            <td>{{floor (($item->times_spent/60)%24)}}</td>
                            <td>{{floor ($item->times_spent%60)}}</td>

                            
                        </tr>
                        <tr>
                        <td>{{ $item->stock_transport->code }}</td>
                        <td>{{$item->stock_transport->created_at}}</td>
                        <td>{{$item->stock_transport->updated_at}}</td>
                        <td>{{$startTime_transport->diffInDays($endTime_transport)}}</td>
                        <td>{{ floor($startTime_transport->diffInHours($endTime_transport) %24)}}</td>
                        <td>{{ floor($startTime_transport->diffInMinutes($endTime_transport) %60) }}</td>

                        

                        </tr>
                        <tr>
                        <td>{{ $item->stock_transport->advance_notice->code }}</td>
                        <td>{{$item->stock_transport->advance_notice->created_at}}</td>
                        <td>{{$item->stock_transport->advance_notice->updated_at}}</td>
                        <td>{{$startTime_advance->diffInDays($endTime_advance)}}</td>
                        <td>{{ floor($startTime_advance->diffInHours($endTime_advance) %24) }}</td>
                        <td>{{ floor($startTime_advance->diffInMinutes($endTime_advance) %60) }}</td>
                        </tr>
                    </tr>
                    @endforeach
                    </tbody>
    ...
    <div class="box">
            ...
            <div class="box-body table-responsive no-padding">
                <table class="table table-bordered table-hover no-margin activity">
                    ...
                    <tbody>
                    @foreach($stockDeliveries as $item)
                    ...
                    <tr>
                        <tr>
                            <td rowspan="4" style="text-align: center; vertical-align: middle">{{$item->warehouse_name}}</td>
                            <td>{{ $item->code }}</td>
                            <td>{{$item->created_at}}</td>
                            <td>{{$item->updated_at}}</td>
                            <td>{{floor ($item->times_spent/1440)}}</td>
                        ...
                        <tr>
                        <td>{{ $item->stock_entry->code }}</td>
                        <td>{{$item->stock_entry->created_at}}</td>
                        <td>{{$item->stock_entry->updated_at}}</td>
                        <td>{{$startTime_entries->diffInDays($endTime_entries)}}</td>
                        <td>{{floor($startTime_entries->diffInHours($endTime_entries)%24)}}</td>
                        <td>{{ floor($startTime_entries->diffInMinutes($endTime_entries) %60) }}</td>

                        

                        </tr>
                        <tr>
                        <td>{{ $item->stock_entry->stock_transport->code }}</td>
                        <td>{{$item->stock_entry->stock_transport->created_at}}</td>
                        <td>{{$item->stock_entry->stock_transport->updated_at}}</td>
                        <td>{{$startTime_transport->diffInDays($endTime_transport)}}</td>
                        <td>{{floor($startTime_transport->diffInHours($endTime_transport)%24)}}</td>
                        <td>{{ floor($startTime_transport->diffInMinutes($endTime_transport) %60) }}</td>
                        ...
                        <tr>
                        <td>{{ $item->stock_entry->stock_transport->advance_notice->code }}</td>
                        <td>{{$item->stock_entry->stock_transport->advance_notice->created_at}}</td>
                        <td>{{$item->stock_entry->stock_transport->advance_notice->updated_at}}</td>
                        <td>{{$startTime_advance->diffInDays($endTime_advance)}}</td>
                        <td>{{floor($startTime_advance->diffInHours($endTime_advance)%24)}}</td>
                        <td>{{ floor($startTime_advance->diffInMinutes($endTime_advance) %60) }}</td>
                        </tr>
...
@if(Auth::user()->hasRole('CommandCenter'))
<div class="row">
		<div class="col-md-6 col-sm-12 col-xs-12">
			<div class="box">
				<div class="box-header">
    ...
    <input type="hidden" value="{{ route('api.good_issue_management_monitoring')  }}" id="good_issue_management">
    ...
    <input type="hidden" value="{{ session()->get('current_project')->id }}" id="project_id">
        ...
        window.onload = function() {
            ...
            var columnChart = new CanvasJS.Chart("inboundChart", {
                ...
                data: [
                    ...
                    //     legendText: "Pending",
                    //     showInLegend: true,
                    //     dataPoints:{!! json_encode($pendingInbound) !!}
                    }]
            });
            columnChart.render();
            ...
            var columnChart = new CanvasJS.Chart("outboundChart", {
                ...
                data: [{
                    ...
                    //     legendText: "Pending",
                    //     showInLegend: true,
                    //     dataPoints:{!! json_encode($pendingOutbound) !!}
                    }]
            });
            columnChart.render();
            ...
            var progressChart = new CanvasJS.Chart("progressChart", {
                ...
                data: [{
                    ...
                    dataPoints: {!! json_encode($progress) !!}
                }]
            });
            ...
            var columnChart = new CanvasJS.Chart("handlingChart", {
                ...
                data: [{
                    ...
                    legendText: "In",
                    showInLegend: true,
                    dataPoints:{!! json_encode($handlingUnit) !!}
                },
                    {
                        type: "column",
                        name: "Out",
                        legendText: "Out",
                        showInLegend: true,
                        dataPoints:{!! json_encode($handlingWeight) !!}
                    }]
            });
            columnChart.render();
